...
class PuzzleCell(MapCell):
	...
	def handleInteraction(self):
		if(self._completed):
			return
		SLEEP = 0.5
		self.print("Congratulations? You found the key to the treasure.")
		time.sleep(SLEEP)
		self.print("Well, almost...")
		time.sleep(SLEEP)
		self.print("The key is in a treasure chest, and it is locked.")
		time.sleep(SLEEP)
		while (True):
			tightness = 0
			...
			if (tightness > 0):
				...
				help = False
				try:
					dial = self.input("Dial to move? [1, 2, 3, q=quit, h=help]? ")
					...
					dial = int(dial)
					if(1 <= dial <= 3):
						num = int(self.input("Move it to? [0 - 9]? "))
				except:
					self.print("Invalid Values")
					pass
				...
				self.print("The chest pops open, revealing an intricate key, which you keep in a safe place.\n")
				self._map.getPlayer()._hasKey = True
